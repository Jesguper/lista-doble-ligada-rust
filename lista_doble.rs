...
impl<T:Copy> List<T> {
    ...
    pub fn push_back(&mut self, value: T){
        ...
        match &mut self.tail.take(){
            None => {
                //Si no tiene nada, a head y tail se les pone el valor de value
                self.head = node.into();
                self.tail = self.head.clone();
            },
            //En caso de que encuentre un valor en tail
            Some(current_tail) => {
                //el valor actual del nodo se pasa al nodo previo
                node.prev = Some(Rc::downgrade(&current_tail));
                //El nuevo valor de tail es el valor de value
                self.tail = node.into();
                //El valor de tail previo toma la referencia debil de tail actual
    ...
    pub fn pop_front(&mut self) -> Option<T>{
        ...
        match &mut self.head.take() {
            ...
            Some(head) =>{
                ...
                //Devuelve el valor de head como Option
                Some(head.value)
            }
        }
    }
    ...
    pub fn pop_back(&mut self) -> Option<T>{
        ...
        match &mut self.tail.take(){
            ...
            Some(tail)=>{
                ...
                //Devuele el valor de tail como Option
                Some(tail.value)
            }
        }
    }
